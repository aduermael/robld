...
description: >
  Develop this Roblox game with robuild, Studio Script Sync (Luau on disk),
  ...
  Luau, Script Sync, playtest, MCP, DataModel, robuild.
---

# Roblox: robuild + Script Sync + Studio MCP

## Bootstrap

Run `robuild` from this folder (`go build -o robuild .`, Windows `go build -o robuild.exe .`). Same command every time.

| Exit | What you do |
|---|---|
| 0 and `READY:` | Treat Script Sync + MCP as on. Continue the task. |
| 1 `NEED_PLACE:` | `robuild --new` / `robuild --new "Name"`, or a place id / game URL. Re-run. |
| 1 `ERROR:` | Show the message (usually Studio missing). Do not invent a connection. |
| 2 `NOT_READY:` | Show the printed Sync to… steps. After the user does them, re-run `robuild`. |

Do not try to start Script Sync from MCP or Luau. Pass ids as args; do not prompt.

Script Sync **preferences** are per-machine; main `robuild` writes them and may restart Studio. **`robuild-sync.json`** is the per-project folder map — honor it; games are not all `ServerScriptService/` on disk. Do not overwrite a custom map. The generated default maps `src/server|shared|client` to **services** (that is what Studio auto-resumes). New Luau goes next to existing siblings. Do not start Sync to… from MCP. If `robuild` exits `NOT_READY`, show the printed Sync to… paths. `robuild dump` → read `robuild-dump/REPORT.txt` and `rbx-storage.txt`.

## What lives where

- **Luau:** files in this tree (Script Sync). Create/edit/delete on disk only.
- **World / instances:** `place.rbxlx`. After MCP creates parts/meshes, run `robuild save` (macOS: File→Save / Cmd+S into Studio). Do not rewrite the XML or restart Studio to persist.
...
6. If the DataModel changed (not just Luau), `robuild save` then confirm `place.rbxlx` changed.
