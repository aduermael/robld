...
- Script Sync Studio Settings live in `GlobalSettings_13.xml`. Studio MCP enable is `Assistant-ExternalMCPEnabled` in `InstalledPlugins/0/settings.json` (and `AssistantSettings/<userId>.json`). Main `robld` writes both and may **restart Studio** on macOS so they apply. `robld dump` writes `robuild-dump/` for inspection (includes `rbx-storage.txt` when that SQLite file exists).
