...
| **2** `NOT_READY:` | Do **Sync to…** in Explorer, then re-run `robld`. |
...
```bash
./cli/robld prefs          # show current values
./cli/robld prefs apply    # write agent defaults (Studio must be closed)
```

`robld` also applies those defaults when it **launches** Studio (not when Studio is already open). Agent defaults:
...
The main `robld` command:

- Writes those prefs (and **restarts Studio** on macOS if it was already open so they take).
- Writes **`robuild-sync.json`** — the per-project Explorer folder ↔ disk folder map. Edit this when a game uses different names (`src/server`, `ReplicatedStorage.Shared`, …). `robld` will not overwrite a map you already saved.
...
Writes `robuild-dump/REPORT.txt` plus copies of GlobalSettings, placeIDEState, plists, and Script Sync log lines. Leave that folder in the project so the agent can read it. `./cli/robld scan` is the stdout-only version.
...
| `place.json` | `robld` (ids, local file path, name) |
| `robuild-sync.json` | `robld` (Studio folder ↔ disk folder map; per project) |
...
Then `./cli/robld` until `READY`. Empty projects get `src/server` ↔ `ServerScriptService` (see `robuild-sync.json`). If auto-resume still does not bind, Studio needs **Sync to…** once per root (Keep Disk). After that, auto-resume + Keep local should bring it back.
...
```bash
./cli/robld https://www.roblox.com/games/123456789/My-Game
./cli/robld --file ./place.rbxlx
```

If you also want that place as a git file: **File → Save to File** as `place.rbxlx` here, then `robld` will prefer it.
...
Studio Settings → Script Sync: `robld prefs apply` (Studio closed) so auto-resume stays on and conflicts keep the files in this repo.
